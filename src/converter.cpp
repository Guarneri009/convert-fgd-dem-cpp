...
auto Converter::calc_image_size(span<const Metadata> meta_data_list) const noexcept
    ...
    // Calculate pixel size from first metadata
    ...
    // Calculate total image size based on bounds and pixel size
    int x_length =
...
auto Converter::combine_meta_data_and_contents(
    ...
    // Place each mesh data into the combined array
    for (size_t i = 0; i < meta_data_list.size() && i < np_array_list.size(); ++i) {
        ...
        // Copy the array data - optimized version
        for (int y = 0; y < y_len && y < static_cast<int>(np_array.size()); ++y) {
            ...
            // Adjust if destination starts before array bounds
            if (dst_start < 0) {
                src_start = -dst_start;
                copy_len += dst_start;
                dst_start = 0;
...
bool Converter::make_data_for_geotiff(std::vector<std::vector<double>> &np_array,
    ...
    geo_transform = {
        bounds.min_lng,  // top left x
        pixel_width,     // pixel width
        0.0,             // rotation
        bounds.max_lat,  // top left y
        0.0,             // rotation
...
bool Converter::run(std::error_code &ec) {
    ...
    if (config_.output_epsg != "EPSG:4326") {
        ...
        if (!geotiff.resampling(config_.output_epsg, resample_ec)) {
            std::cerr << "Warning: Resampling failed\n";
        }
    }

    std::cout << "Output saved to: " << output_file.string() << "\n";
    return true;
}
