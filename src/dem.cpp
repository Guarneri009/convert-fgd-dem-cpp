...
Dem::Dem(std::filesystem::path import_path, bool sea_at_zero)
    ...
    if (!std::filesystem::exists(this->import_path)) {
        std::stringstream ss;
        ss << "File not found: " << this->import_path.string();
...
void Dem::get_xml_content() {
    ...
    if (xml_paths.empty()) {
        throw std::runtime_error("No XML files found in the archive");
    }
...
void Dem::unzip_dem() {
    ...
    if (zip::is_zip_file(import_path)) {
        ...
        if (!result) {
            std::stringstream ss;
            ss << "Failed to extract: " << import_path.string();
            throw std::runtime_error(ss.str());
        }
        ...
        for (const auto &file : *result) {
            if (zip::is_zip_file(file)) {
                ...
                if (!nested_result) {
                    std::cerr << "Failed to extract nested zip: " << file.string() << "\n";
                }
            }
        }
    }
...
void Dem::check_mesh_codes() {
    ...
    if (std::distance(sorted_codes.begin(), last) !=
        ...
        std::cerr << "Warning: Duplicate mesh codes found\n";
    }
}
