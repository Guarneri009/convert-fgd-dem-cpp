...
GitHub Releasesで配布されるビルド済み実行ファイルは、GDALやTBBなどの依存関係がすべて静的リンクされています。そのため、**別途GDALをインストールする必要はありません**。ダウンロードしてすぐに使用できます。
...
- `gdal-data/` - GDAL設定ファイル
- `proj-data/` - PROJ座標変換データ

**使用方法:**
1. GitHubのReleasesページから対応プラットフォームのアーカイブをダウンロード
2. アーカイブを展開
3. そのまま実行可能（GDALのインストール不要）
...
**注意事項:**
- `gdal-data`と`proj-data`フォルダは実行ファイルと同じディレクトリに配置してください
- 別の場所に配置する場合は環境変数`GDAL_DATA`、`PROJ_LIB`でパスを指定できます
